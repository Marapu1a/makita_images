...
SOURCE_IMPORT_DIR = BASE_DIR.parent / "iterations" / "new_items_2026-05-09" / "makita_exact_tail_2026-05-10" / "output" / "import_images"
SOURCE_REPORT_XLSX = BASE_DIR.parent / "iterations" / "new_items_2026-05-09" / "makita_exact_tail_2026-05-10" / "output" / "makita_exact_tail_report.xlsx"
...
def main() -> None:
    ...
    exact_report = pd.read_excel(SOURCE_REPORT_XLSX)
    exact_report = exact_report[exact_report["status"].astype(str).str.upper() == "OK"].copy()

    updated = 0
    for _, row in exact_report.iterrows():
        ...
        report.loc[mask, "source"] = row["source"]
        report.loc[mask, "source_url"] = row["source_url"]
    ...
    remaining = int((report["placeholder_used"].astype(str).str.upper() == "YES").sum())
