...
def build_web_path(article: str, filename: str) -> str:
    return f"/final_delivery_2026-04-09/import_images/{article}/{filename}"
...
def get_preview_and_gallery(folder: Path, article: str) -> tuple[str, str]:
    ...
    preview_path = build_web_path(article, preview_file.name)
    gallery_paths = [build_web_path(article, file.name) for file in gallery_files]
    return preview_path, ";".join(gallery_paths)


def get_bitrix_preview_and_gallery(folder: Path, article: str) -> tuple[str, str]:
    image_files = [p for p in folder.iterdir() if is_image_file(p)]
    if not image_files:
        return "", ""
    ...
    preview_path = build_bitrix_web_path(article, preview_file.name)
    gallery_paths = [build_bitrix_web_path(article, file.name) for file in gallery_files]
    return preview_path, ";".join(gallery_paths)
...
def main() -> None:
    ...
    for _, row in report_df.iterrows():
        ...
        folder = IMAGES_DIR / article
        if not folder.exists():
            continue

        preview_path, more_photo_path = get_preview_and_gallery(folder, article)
        if not preview_path:
            continue
    ...
    for _, row in report_df.iterrows():
        ...
        preview_path, more_photo_path = get_bitrix_preview_and_gallery(folder, article)
        if not preview_path:
            continue
