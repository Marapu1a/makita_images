...
OUTPUT_FILE = BASE_DIR / "accessories_without_images.xlsx"
...
TARGET_SECTION = "Расходные материалы и аксессуары"
...
def main() -> None:
    ...
    section_mask = work_df[MAIN_SECTION_COL].fillna("").astype(str).str.strip() == TARGET_SECTION
    ...
    print("\n=== ACCESSORIES FILTER SUMMARY ===")
    print(f"Input rows: {len(df)}")
    print(f"Target section rows: {int(section_mask.sum())}")
    print(f"Rows without images in target section and with article: {len(filtered_df)}")
