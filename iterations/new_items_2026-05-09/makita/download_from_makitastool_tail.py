...
REPORT_FILE = BASE_DIR / "output" / "makitastool_tail_report.xlsx"

BASE_URL = "https://www.makitastool.com"
SEARCH_API = BASE_URL + "/wp-json/wp/v2/product?search={query}&per_page=10"
SOURCE_NAME = "makitastool.com"
...
def search_product(article: str, session: requests.Session) -> tuple[str | None, str]:
    url = SEARCH_API.format(query=article)
    ...
    for item in data:
        ...
        blob_token = normalize_token(blob)
        if article_token in blob_token:
            exact_candidates.append((link, title))
        elif any(part and normalize_token(part) in blob_token for part in article.split("-")):
            loose_candidates.append((link, title))
...
def canonicalize_image_url(url: str) -> str:
    url = re.sub(r"-\d+x\d+(?=\.[a-z0-9]+$)", "", url, flags=re.I)
    return url
