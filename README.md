...
- ещё позже был закрыт отдельный поздний блок из папки `Результат скриптов`
- отдельно был собран служебный импортный файл для раздела `Запасные части`
...
## Проверенные числа на старт

Актуальная сверка по финальным импортным Excel и `.webp` в товарных папках:

| Блок | Товаров / строк импорта | Реальные картинки | Placeholder | Импортных `.webp` |
| --- | ---: | ---: | ---: | ---: |
...
Для подсчёта файлов считаются только импортные `.webp`, на которые ссылаются Excel-файлы. Технические `.gitkeep` и служебные placeholder-файлы в корне папок поставок не входят в эти числа.
